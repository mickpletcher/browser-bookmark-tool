...
Chrome and Edge `Bookmarks` files, Firefox `places.sqlite` databases, JSON, SQLite, and HTML backups, restore snapshots, generated task scripts, and profile mapping files can contain private browsing data, internal URLs, access tokens, usernames, or local paths. Keep them outside the repository. The project `.gitignore` blocks standard private filenames as a secondary safeguard.
