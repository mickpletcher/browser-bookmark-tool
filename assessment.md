...
| Automated tests | Passing | All 129 test cases pass on Python 3.13.3 across Firefox discovery, Places import and export, cross-browser matching, backup ordering, manifests, disabled-mode isolation, three-browser rollback, merge, machine-readable preview reports and policy results, reusable policies and policy gates, mapping, backup catalog and comparison, backup verification, restore, integrity, logging, scheduling, automation, transaction, process, CLI, and GUI paths. |
...
| Upgrade tracking | Good | Completed `FUT-022` is recorded with replacement candidate `FUT-023` for preview result cataloging and trend summaries. Three priority tiers still track SignPath integration, release provenance, recovery, reporting, macOS Chrome and Edge compatibility, and phased Safari support. |
...
- Atomically writes versioned preview policy results with source hashes, mapping names, counts, limits, violations, and exit codes while excluding bookmark fields and local paths.
