...
| Automated tests | Passing | All 152 test cases pass on Python 3.12, including read-only Safari discovery, parsing, backup, merge/export, privacy, GUI/CLI paths, isolated Firefox verification and restore, macOS compatibility, and Windows regressions. |
...
| macOS compatibility | Ready from source | Chrome, Edge, and Firefox support remains unchanged. Safari version 1 bookmark plists are supported read-only with iCloud warnings; direct Safari writes remain open. Physical-Mac standard-location discovery passed, but live plist reading was denied by macOS privacy controls without additional user-granted access. |
