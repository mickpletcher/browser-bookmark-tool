...
| macOS compatibility | Ready from source | Chrome, Edge, and Firefox support remains unchanged. Safari version 1 bookmark plists are supported read-only with iCloud warnings and a guided, user-controlled HTML import workflow; direct automated Safari writes remain open. A physical Mac read and validated an isolated backup containing 285 bookmarks and 61 folders, verified its manifest, and confirmed the live plist hash was unchanged. |
